...
# Path to the motor — honors $DOTFILES_DIR (set by dev-bootstrap when the
# 95-dotfiles-personal topic clones the user's fork) so non-default repo
# locations work without code edits.
_AUTO_UPDATE_MOTOR="${DOTFILES_DIR:-$HOME/dotfiles}/scripts/auto-update.sh"
...
if ! autoload -Uz add-zsh-hook 2>/dev/null; then
    "$_AUTO_UPDATE_MOTOR" --from-shell-start
    return 0
fi
